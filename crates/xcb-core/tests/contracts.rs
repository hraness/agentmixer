...
fn adaptive_and_fusion_keep_exact_provider_issued_identifiers() {
    ...
    assert!(
        choices
            .iter()
            .all(|choice| choice.provider == Provider::Devin)
    );
}
...
fn runway_is_an_estimate_from_comparable_fresh_quota_observations() {
    ...
    let Estimate::Known { seconds } = estimate else {
        panic!("missing estimate");
    };
    assert!((seconds - 420.0).abs() < 0.001);
    assert!(matches!(
        runway(&[quota(70_000, 30.0)], 70_000),
        Estimate::Unknown { .. }
    ));
    assert!(matches!(
        runway(&[quota(10_000, 30.0), quota(70_000, 20.0)], 70_000),
        Estimate::Unknown { .. }
    ));
    assert!(matches!(
        runway(&[quota(10_000, 20.0), quota(70_000, 30.0)], 800_000),
        Estimate::Unknown { .. }
    ));
    assert!(matches!(
        runway(&[quota(10_000, f64::NAN), quota(70_000, 30.0)], 70_000),
        Estimate::Unknown { .. }
    ));
}
...
fn pane_presets_roundtrip_without_executable_authority() {
    ...
    assert!(
        Pane::parse(
            br#"{"version":2,"id":"v2","title":"x","root":{"type":"text","value":"hello"}}"#
        )
        .is_err()
    );
}
