use crate::{Error, Result, attachments, auth, config::Config, digest, new_id, now_ms, panes, process::Pin, runner::{self, Observer, Outcome, Progress, RunInput}, store::Store, summary};
use std::{collections::{BTreeMap, BTreeSet}, path::{Path, PathBuf}, sync::{Arc, Mutex, mpsc::{Receiver, SyncSender, TryRecvError}}, time::Duration};
use tokio::{sync::{mpsc, watch}, task::JoinHandle};
use xcb_core::{Id, Provider, models::{ModelChoice, Preference}, panes::Pane, policy::{RouteCandidate, Terminal, next_route, should_continue}, session::{Message, Role, Session, State, Subagent}, ui::{Intent, Update}};

pub fn choose_model(store: &Store, provider: Provider, requested: Option<&str>, config: &Config) -> Result<ModelChoice> {
    let mut choices: Vec<_> = store.models()?.into_iter().filter(|choice| choice.provider == provider).collect();
    xcb_core::models::sort_choices(&mut choices, &config.favorites);
    if let Some(requested) = requested {
        let matches: Vec<_> = choices.into_iter().filter(|choice| choice.key() == requested || choice.id.as_str() == requested || choice.label == requested).collect();
        return match matches.as_slice() { [choice] => Ok(choice.clone()), [] => Err(Error::Unavailable("model not observed; refresh the catalog")), _ => Err(Error::Unavailable("model is ambiguous; use its full provider/model/effort key")) };
    }
    choices.into_iter().next().ok_or(Error::Unavailable("no observed models; run xcb models refresh for this provider"))
}

pub fn new_session(store: &Store, workspace: &Path, config: &Config, account: Option<&Id>, model: Option<&str>) -> Result<Session> {
    let id = account.or(config.default_account.as_ref()).cloned().or_else(|| store.accounts().ok()?.into_iter().find(|account| account.enabled).map(|account| account.id)).ok_or(Error::Unavailable("add an account with xcb accounts add, then sign in"))?;
    ...
    store.session(&session.id)?.ok_or(Error::Unavailable("session not found"))
}

fn ready(store: &Store, session: &Session) -> Result<()> {
    if session.model.provider != Provider::Claude { return Err(Error::Unavailable("native execution for this provider is still unqualified; catalog entries are not activation")); }
    Pin::load(store.root(), session.model.provider)?;
    if !auth::has_token(store, &session.account)? { return Err(Error::Unavailable("sign in with xcb accounts login before running a task")); }
    if store.unsettled_runs()?.iter().any(|run| run.account == session.account) { return Err(Error::Conflict("account has an unsettled run")); }
    Ok(())
}

pub async fn execute(store: Arc<Store>, session_id: Id, text: String, attachments: Vec<xcb_core::session::Attachment>, pane_generation: bool, cancel: watch::Receiver<bool>, observer: Observer) -> Result<Outcome> {
    let started = now_ms();
    ...
    loop {
        if *cancel.borrow() { return Err(Error::Unavailable("cancelled before the next turn")); }
        let config = Config::load(store.root())?.0;
        let session = store.session(&session_id)?.ok_or(Error::Unavailable("session not found"))?;
        ready(&store, &session)?;
        tried.insert(format!("{}/{}", session.account, session.model.key()));
        let message = Message { id: new_id("m"), role, text, attachments, at_ms: now_ms() };
        let current = store.append_message(&session_id, session.revision, &message)?;
        let outcome = runner::run(store.clone(), RunInput { session: current, message, config: config.clone(), pane_generation }, cancel.clone(), observer.clone()).await?;
        if pane_generation || *cancel.borrow() { return Ok(outcome); }
        let current = store.session(&session_id)?.ok_or(Error::Unavailable("session not found"))?;
        if current.account != session.account || current.model.key() != session.model.key() { return Ok(outcome); }
        ...
        if should_continue(&current_config.extensions.auto_continue, &outcome.facts, consecutive, now_ms().saturating_sub(started), repeat) {
            consecutive += 1;
            previous_output = Some(output_digest);
            observer(Progress::Notice(format!("Auto-continue {consecutive}/{} · same task and permissions", current_config.extensions.auto_continue.max_consecutive)));
        ...
        if current_config.auto_failover && now_ms().saturating_sub(started) < current_config.extensions.auto_continue.max_elapsed_ms {
            let view = summary::snapshot(&store, Some(&session_id), &current_config, now_ms())?;
            let source = RouteCandidate { account: current.account.clone(), model: current.model.clone(), admitted: true, quota_fresh: true, available: false };
            let mut candidates = Vec::new();
            for model in &view.models {
                for account in &view.accounts {
                    if account.provider != model.provider || account.busy || !account.enabled || candidates.len() >= 256 { continue; }
                    candidates.push(RouteCandidate { account: account.id.clone(), model: model.clone(), admitted: model.provider == Provider::Claude && Pin::load(store.root(), Provider::Claude).is_ok(), quota_fresh: account.remaining_percent.is_some(), available: account.remaining_percent.is_some_and(|remaining| remaining > 0.0) });
                }
            }
            if let Some(target) = next_route(&source, &candidates, &tried, &outcome.facts, !outcome.text.is_empty() || outcome.facts.effects == xcb_core::policy::EffectState::None) {
                store.rebind(&session_id, current.revision, &target.account, target.model.clone())?;
                observer(Progress::Notice(format!("Usage limit: continuing the checkpoint on {} · {}", target.model.provider, target.model.label)));
...
#[derive(Default)]
struct Activity { tools: Vec<String>, subagents: BTreeMap<Id, Subagent> }
struct Active { cancel: watch::Sender<bool>, task: JoinHandle<()>, activity: Arc<Mutex<Activity>>, pane: bool }

fn publish(store: &Store, current: Option<&Id>, config: &Config, active: &BTreeMap<Id, Active>, output: &SyncSender<Update>) -> Result<()> {
    ...
    if let Some(active) = current.and_then(|id| active.get(id)) {
        ...
        if let Ok(activity) = active.activity.lock() { view.activity = activity.tools.clone(); view.subagents = activity.subagents.values().cloned().collect(); }
    } else if view.state == State::Working { view.state = State::Uncertain; }
    let _ = output.try_send(Update::View(Box::new(view)));
    Ok(())
}

fn start(store: Arc<Store>, id: Id, text: String, attachments: Vec<xcb_core::session::Attachment>, pane: bool, output: SyncSender<Update>, finished: mpsc::Sender<(Id, Result<Outcome>)>) -> Active {
    ...
    let observer: Observer = Arc::new(move |event| {
        match event {
            Progress::Text { thinking, text } if !pane => { let _ = output.try_send(Update::Delta { session: session_id.clone(), thinking, text }); }
            Progress::Tool(name) => if let Ok(mut activity) = activity_copy.lock() { if activity.tools.len() >= 128 { activity.tools.remove(0); } activity.tools.push(name); },
            Progress::Subagent(agent) => if let Ok(mut activity) = activity_copy.lock() { if activity.subagents.len() < 64 || activity.subagents.contains_key(&agent.id) { activity.subagents.insert(agent.id.clone(), agent); } },
            Progress::Notice(message) => { let _ = output.try_send(Update::Notice(message)); }
            _ => (),
        }
    });
    let task = tokio::spawn(async move { let result = execute(store, id.clone(), text, attachments, pane, cancelled, observer).await; let _ = finished.send((id, result)).await; });
    Active { cancel, task, activity, pane }
}

pub async fn serve(store: Arc<Store>, workspace: PathBuf, mut current: Option<Id>, input: Receiver<Intent>, output: SyncSender<Update>) -> Result<()> {
    let mut config = Config::load(store.root())?.0;
    let mut active: BTreeMap<Id, Active> = BTreeMap::new();
    let (completed, mut completions) = mpsc::channel(16);
    ...
    loop {
        tokio::select! {
                ...
                if let Some((queued_id, request)) = pending_pane.take() {
                    if !quit { let session = store.session(&queued_id)?.ok_or(Error::Unavailable("session not found"))?; let generated = generation_session(&store, &session, &config)?; let task = start(store.clone(), generated.id.clone(), pane_prompt(&request)?, vec![], true, output.clone(), completed.clone()); active.insert(generated.id, task); }
                }
                publish(&store, current.as_ref(), &config, &active, &output)?;
            },
            _ = ticker.tick() => {
        ...
        if quit && active.is_empty() { break; }
    }
    for (_, task) in active { let _ = task.cancel.send(true); let _ = task.task.await; }
