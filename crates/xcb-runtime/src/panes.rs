...
use xcb_core::{
    Id,
    panes::{MAX_PANE_BYTES, Pane},
};

pub fn list(root: &Path) -> Result<Vec<Pane>> {
    let mut panes: BTreeMap<Id, Pane> = Pane::presets()
        .into_iter()
    ...
    for entry in fs::read_dir(directory)? {
        ...
        count += 1;
        if count > 128 {
            return Err(xcb_core::Error::Limit("pane files").into());
        }
        if entry
            .path()
            .extension()
        ...
        if let Ok(pane) = private::read(&entry.path(), MAX_PANE_BYTES)
            .and_then(|bytes| Pane::parse(&bytes).map_err(Into::into))
            && entry.file_name().to_str() == Some(&format!("{}.json", pane.id))
        {
...
pub fn load(root: &Path, id: &Id) -> Result<(Pane, Option<String>)> {
    ...
    match private::read(&path, MAX_PANE_BYTES) {
        Ok(bytes) => {
            let pane = Pane::parse(&bytes)?;
            if &pane.id != id {
                return Err(Error::Conflict("pane file identity mismatch"));
            }
            Ok((pane, Some(digest(bytes))))
        }
        Err(Error::Io(error)) if error.kind() == std::io::ErrorKind::NotFound => Pane::presets()
            .into_iter()
            .find(|pane| &pane.id == id)
            .map(|pane| (pane, None))
            .ok_or(Error::Unavailable("pane not found")),
        Err(error) => Err(error),
    }
}
