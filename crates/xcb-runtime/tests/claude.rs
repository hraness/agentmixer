...
fn quota_failures_are_not_guessed_from_arbitrary_assistant_prose() {
    ...
    assert!(matches!(parse_event(prose).unwrap(), Event::Assistant { .. }));
    ...
    assert!(matches!(parse_event(limit).unwrap(), Event::Quota { failure: Some(Failure::AccountQuota), .. }));
}
...
fn stream_deltas_and_terminal_failures_have_distinct_types() {
    ...
    assert!(matches!(parse_event(delta).unwrap(), Event::Delta { thinking: false, ref text } if text == "hello"));
    let end = br#"{"type":"result","subtype":"success","is_error":false,"result":"done","stop_reason":"end_turn"}"#;
    assert!(matches!(parse_event(end).unwrap(), Event::Result { terminal: Terminal::Completed, .. }));
    let failed = br#"{"type":"result","subtype":"success","is_error":true,"result":"blocked"}"#;
    assert!(matches!(parse_event(failed).unwrap(), Event::Result { terminal: Terminal::Failed, .. }));
}
...
fn malformed_recognized_events_refuse_and_unknown_events_are_inert() {
    ...
    assert!(matches!(parse_event(br#"{"type":"future_notification"}"#).unwrap(), Event::Notice));
