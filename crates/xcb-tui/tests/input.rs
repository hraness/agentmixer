...
fn bracketed_paste_preserves_multiline_text_without_submitting() {
    ...
    assert!(matches!(composer.handle(Event::Paste("first\nsecond".into())), ComposerAction::None));
    assert_eq!(composer.text(), "first\nsecond");
    let submit = composer.handle(Event::Key(KeyEvent::new(KeyCode::Enter, KeyModifiers::NONE)));
...
fn alt_enter_is_a_newline_and_unicode_editing_is_safe() {
    ...
    composer.handle(Event::Key(KeyEvent::new(KeyCode::Backspace, KeyModifiers::NONE)));
    assert_eq!(composer.text(), "λ東京\nnex");
}
...
fn cancelling_never_submits_and_clipboard_has_its_own_action() {
    ...
    assert!(matches!(composer.handle(Event::Key(KeyEvent::new(KeyCode::Char('c'), KeyModifiers::CONTROL))), ComposerAction::Cancel));
    assert!(composer.text().is_empty());
    assert!(matches!(composer.handle(Event::Key(KeyEvent::new(KeyCode::Char('v'), KeyModifiers::CONTROL))), ComposerAction::Clipboard));
}
