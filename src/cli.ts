...
function parseFlags(args: readonly string[]): { provider: CliProviderName; model: string | undefined; positional: string[]; prompt: string | undefined } {
  const positional: string[] = [];
  let provider: CliProviderName = "claude", model: string | undefined, prompt: string | undefined;
  for (let i = 0; i < args.length; i += 1) {
    const arg = args[i]!;
    if (arg === "--provider" || arg === "--model" || arg === "-p") {
      const value = args[++i];
  ...
  return { provider, model, positional, prompt };
}
...
async function commandAuth(provider: string, stateRoot: string): Promise<number> {
  ...
  const status = await claudeAuthStatus(stateRoot, inspection);
...
async function commandRun(prompt: string, workspace: string, stateRoot: string, provider: CliProviderName, model: string | undefined): Promise<number> {
  ...
  const opened = await openCliProvider(stateRoot, provider, profile);
  ...
  try {
    ...
    if (result.outcome.status !== "completed") {
      process.stderr.write(`${red("agentmixer:")} ${result.outcome.code ?? "run failed"}\n`);
      return 1;
    }
...
export async function main(argv: readonly string[]): Promise<number> {
  ...
  if (command === "auth") return await commandAuth(rest[0] ?? fail("usage: agentmixer auth claude"), stateRoot);
  ...
  if (command === "run") {
    ...
    return await commandRun(boundedText(prompt, 512 * 1024), process.cwd(), stateRoot, flags.provider, flags.model);
  }
  if (command === "resume") {
    const flags = parseFlags(rest);
    const id = flags.positional[0];
    if (id === undefined || flags.positional.length > 1) return fail("usage: agentmixer resume <session-id>");
